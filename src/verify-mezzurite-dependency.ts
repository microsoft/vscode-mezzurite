...
export class MezzuriteDependency {
  ...
  public async verifyMezzuriteDependency () {
    ...
    let dependencyFound = await MezzuriteDependency.searchInPackageJsonFile();
  ...
  static async searchInPackageJsonFile () {
    let packageJsonFiles = await MezzuriteUtils.searchWorkspace(workspace, ExtensionConstants.pathForAppsPackageJson, ExtensionConstants.pathForNodeModules);
  ...
  static getMezzuriteDependency (dependancyObject: any) {
    let mezzuriteDependency: any = undefined;
    for (let dependency in dependancyObject) {
  ...
  static async searchNodeDirectory () {
    let mezzuriteLibs;
    let mezzuFrameworkName: any = undefined;
    ...
    if (mezzuFrameworkName !== undefined) {
      let mzzFrameworksPackageJsonPath = '**' + ExtensionConstants.mezzuritePath + mezzuFrameworkName + '/package.json';
