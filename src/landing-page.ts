...
export class LandingPage {
  ...
  public getStyles (): string {
    ...
    let css = `<link rel="stylesheet" type="text/css" href="${baseStyle}">`;
    return css;
  }
  ...
  public getMiscPath (file: string, asResource: boolean): string {
    if (asResource) {
      return vscode.Uri.file(this.context.asAbsolutePath(path.join(ExtensionConstants.clientFolderName, file))).with({ scheme: 'vscode-resource' }).toString();
    }
